...
%
As shown in figure~\ref{fig:dense_sparse_reg}, region $D$ within $C_{b}$ contains more nodes or is more dense than its complement $D^{c}$.
%
...
%
As depicted in figure~\ref{}, counting the number of neighbors within the communication area of nodes is not enough, we have to find the region $H_{D}$ with at least $d$ nodes such that its sparse complement $H_{D}^{C}$ contains less than $d-1$ nodes.
%
The orientation of such $H_{D}$ is a concern too, as figure~\ref{} depicts, the most dense region in $C_{B1}$ points towards west while $C_{B2}$ points to north.
\raziel{here you need a comment about what branch in math study this kind of problems. Continue digging into: minimal enclosing circle, circle packing, spatial analysis, etc...}

Let us consider how to find $H_{D}$ in a scenario where one node has two neighbors.
%
As figure~\ref{} shows, we observe that $\alpha_{0}$ is the biggest angle with a size in radians equal to $\alpha_{0} = 2\pi - \measuredangle n_{2}n_{A}n_{1}$. Given $\alpha_{0} > \pi$ there exist an infinite number of half disks with an sparse density within $\alpha_{0}$.
%
For our proposes, it is enough to define just one sparse region $H_{D}^{C}$ of $\pi$ radians.
%
In order to find $H_{D}^{C}$ we split $\alpha_{0}$ in two equal parts by drawing a line $l_{1}$ that 
