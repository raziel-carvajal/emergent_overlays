...
%
In particular, their efficiency can be achieved from
...
\begin{itemize}
    ...
    that receive a broadcast message. Higher the coverage is, better
    it is.
...
%
All these algorithms implement the broadcast with a form of
\emph{controlled flooding} to reach a specific tradeoff: 
since the transmission range of a source node is typically much
smaller than the network deployment area, broadcasting is achieved by
...
\begin{itemize}
	...
	\item \emph{ABBA} (\textit{Area Based Beacon less Algorithm}) controls the dissemination process in time, by setting up timers that must elapse with no duplicate reception to trigger the forwarding.
