...
The power consumption depends on the number of messages that nodes exchange.
%
The amount of received and sent messages is important because, every time a node sends or receives a message, the \textit{transmitter} and/or \textit{receiver} components of its radio change to a more power hungry mode.
%
Hence, the broadcast messages retransmitted as well as the control messages have a direct influence on energy consumption.
%
However, the \textit{network saturation in the vicinity} of the node also influences the energy consumption due to the way the MAC layer is implemented.
...
%
This is interesting because it indicates that properly using control messages to build an underlying topology is worth in terms of power consumption when there is no mobility \deleted{the interval among broadcast messages is close to the interval among control messages}.
%
...
%
Indeed, since the location of each node changes continuously, ABBA occasionally fails in properly computing the covered area; as a consequence, some messages are not delivered.
This result is consistent with previous studies~\cite{YiGerla2003, Garbinato2010}. 
