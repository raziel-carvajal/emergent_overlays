...
Nodes in denser and more ``static'' areas (i.e. around points of interest) \emph{autonomously} decide to switch from the controlled flooding protocol to overlay-based broadcast.
%
...
Instead, we rely on passive observation of information collected from the regular operation of the broadcast and MAC protocols running on each node. 
...
\textbf{Adaptation.}
%
An emergent overlay is controlled by an adaptation policy to decide  when to emerge and when to dissolve. 
Policies must be decentralized: there is no global vision of the MANET or centralized decision-making.
We only allow specific information to be piggybacked over existing messages to enable coordinated decisions between a cluster of nodes. %%% ER: remove if we do not have this in the end in the policies
