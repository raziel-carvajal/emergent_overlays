...
Nodes in denser and more ``static'' areas (i.e. around points of interest) \emph{autonomously} decide to switch from the controlled flooding protocol and switch to overlay-based broadcast.
%
...
\textbf{Adaptation.}
%
An emergent overlay is controlled by an adaptation policy to decide on when to emerge and when to dissolve. 
Policies must be decentralized: There is no global vision of the MANET or centralized decision-making.
We only allow specific information be piggybacked over existing messages to enable coordinated decisions between a cluster of nodes. %%% ER: remove if we do not have this in the end in the policies
