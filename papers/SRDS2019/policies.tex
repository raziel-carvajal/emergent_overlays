...
The emergence of an overlay is driven by observed density and mobility.
...
The use of extra messages go against our objective of reducing costs and risks of collisions.
...
While this \emph{single-node} policy will lead homogeneously dense and stable regions to eventually switch to MPR, it has the risk of creating small regions using MPR surrounded by CF zones, when nodes density is distributed between ${d}_{\text{keep}}$ and ${d}_{\text{emerge}}$.

\input{policy_algo}

A \emph{coordinated} policy is given in Algorithm~\ref{algorithm:coordinated}.
This policy targets two objectives:
First, it should allow entire \emph{regions} of the MANET to switch simultaneously to the use of MPR, and thereby avoid single isolated nodes using an overlay.
Second, it must allow created overlays to gracefully \emph{dissolve} when they are no longer necessary, without unnecessarily keeping isolated remainder regions.
...
After starting sending \texttt{hello} frames (line 18), nodes wait for a period of $2\times\Delta$ before switching to the MPR protocol (line 19).
%
The lifetime of an MPR overlay is governed by the timer $t_\text{MPR}$ at each node.
This timer is always set to a duration of $10\times\Delta$ when a node switches to, or continues using, MPR (line 20).
...
This accounts for mobile nodes who have moved out of a dense zone, and are no longer in range of any node using MPR.
