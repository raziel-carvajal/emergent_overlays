...
Nodes keep using MPR when their local observables of density and stability $(\bar{s}_{-\delta},\bar{d}_{-\delta})$ exceed a fixed threshold $({s}_{\text{min}},{d}_{\text{emerge}})$ otherwise, nodes switch to CF.
This simple policy of adaptation results in having an overlay in dense and stable regions but it has the risk of creating small overlays surrounded by CF zones, when nodes density varies between ${d}_{\text{keep}}$ and ${d}_{\text{emerge}}$.
A \emph{coordinated} mechanism is given in Algorithm~\ref{algorithm:coordinated}, targeting two objectives:
