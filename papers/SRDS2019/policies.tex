...
Instead, we observe that the necessary information can be collected passively from the MAC protocol, BMW~\cite{tang2001mac}\laurent{what is BMW?}.
...
In order to avoid oscillations where nodes would switch back and forth between using CF and MPR, we use an hysteresis, with two pairs of thresholds: 
...
%
Other nodes receiving a message with \texttt{em} set will check whether their local observables are above the $({s}_{\text{min}},{d}_{\text{keep}})$ thresholds (line 14).
...
%
Nodes that do not pass any of the thresholds unset \texttt{em}, marking the boundary of the emerging overlay region (line 15).
