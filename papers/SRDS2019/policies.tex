...
We now present how overlays emerge autonomously in a system where all nodes initially use controlled flooding.
We start by presenting how nodes leverage the running protocols to gather information about their environment and surrounding.
The collected \emph{observables} are used by an adaptation \emph{policy}, that implements the decision to switch.
The decision is taken collectively by nodes in a fully decentralized way, by piggybacking best-effort consensus messages over existing protocol messages.
