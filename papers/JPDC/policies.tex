...
%~\citep{2001:AAR:876878.879323,adaptive_group_com_iscc02}

The emergence of an overlay is driven by the observed density and mobility.
Those could be measured with periodic probing messages, as used by Tseng et~al.~\citep{2001:AAR:876878.879323} and Viswanath and Obraczka~\citep{adaptive_group_com_iscc02} to adapt the parameters of controlled flooding.
...
A simple policy would be to let nodes decide independently when to switch from CF to MPR, when the locally observed value for $(\bar{s}_{-\delta},\bar{d}_{-\delta})$ exceeds both thresholds $({s}_{\text{min}},{d}_{\text{emerge}})$.
As long as these values remain over thresholds $({s}_{\text{min}},{d}_{\text{keep}})$, a node would continue using MPR, but switch back to CF otherwise.
...
A \emph{coordinated} mechanism is given in Algorithm~\ref{algorithm:coordinated}, that addresses these shortcomings.
First, it allows entire \emph{regions} of the MANET to switch simultaneously to use MPR, thereby avoiding single isolated nodes using an overlay.
Second, it allows the created overlays to gracefully \emph{dissolve} when they are no longer necessary, without unnecessarily keeping isolated residual regions.
...
This accounts for mobile nodes who have moved out of a dense zone, and are no longer in range of any MPR node.
...
% %\citep{closure_coef}: local closure coefficient used as metric of density in our policies of adaptation
