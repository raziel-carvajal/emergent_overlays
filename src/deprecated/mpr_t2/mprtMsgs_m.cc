...
NeighborsDescriptor::NeighborsDescriptor() : cClassDescriptor("inet::mpr_t2::Neighbors", "cPacket")
{
}
...
RequestNeighborsDescriptor::RequestNeighborsDescriptor() : cClassDescriptor("inet::mpr_t2::RequestNeighbors", "cPacket")
{
}
...
Register_Class(MprFound);

MprFound::MprFound(const char *name, int kind) : ::cPacket(name,kind)
{
    this->sender_var = 0;
    inMpr_arraysize = 0;
    this->inMpr_var = 0;
...
MprFound::MprFound(const MprFound& other) : ::cPacket(other)
{
    inMpr_arraysize = 0;
    this->inMpr_var = 0;
    copy(other);
...
MprFound& MprFound::operator=(const MprFound& other)
{
    if (this==&other) return *this;
    ::cPacket::operator=(other);
    copy(other);
...
void MprFound::copy(const MprFound& other)
{
    this->sender_var = other.sender_var;
    delete [] this->inMpr_var;
    this->inMpr_var = (other.inMpr_arraysize==0) ? NULL : new opp_string[other.inMpr_arraysize];
    inMpr_arraysize = other.inMpr_arraysize;
    for (unsigned int i=0; i<inMpr_arraysize; i++)
        this->inMpr_var[i] = other.inMpr_var[i];
}

void MprFound::parsimPack(cCommBuffer *b)
{
    ::cPacket::parsimPack(b);
    doPacking(b,this->sender_var);
    b->pack(inMpr_arraysize);
    doPacking(b,this->inMpr_var,inMpr_arraysize);
}

void MprFound::parsimUnpack(cCommBuffer *b)
{
    ::cPacket::parsimUnpack(b);
    doUnpacking(b,this->sender_var);
    delete [] this->inMpr_var;
    b->unpack(inMpr_arraysize);
    if (inMpr_arraysize==0) {
        this->inMpr_var = 0;
    } else {
        this->inMpr_var = new opp_string[inMpr_arraysize];
        doUnpacking(b,this->inMpr_var,inMpr_arraysize);
    }
}

const char * MprFound::getSender() const
{
    return sender_var.c_str();
}

void MprFound::setSender(const char * sender)
{
    this->sender_var = sender;
}

void MprFound::setInMprArraySize(unsigned int size)
{
    opp_string *inMpr_var2 = (size==0) ? NULL : new opp_string[size];
    unsigned int sz = inMpr_arraysize < size ? inMpr_arraysize : size;
    for (unsigned int i=0; i<sz; i++)
        inMpr_var2[i] = this->inMpr_var[i];
    for (unsigned int i=sz; i<size; i++)
        inMpr_var2[i] = 0;
    inMpr_arraysize = size;
    delete [] this->inMpr_var;
    this->inMpr_var = inMpr_var2;
}

unsigned int MprFound::getInMprArraySize() const
{
    return inMpr_arraysize;
}

const char * MprFound::getInMpr(unsigned int k) const
{
    if (k>=inMpr_arraysize) throw cRuntimeError("Array of size %d indexed by %d", inMpr_arraysize, k);
    return inMpr_var[k].c_str();
}

void MprFound::setInMpr(unsigned int k, const char * inMpr)
{
    if (k>=inMpr_arraysize) throw cRuntimeError("Array of size %d indexed by %d", inMpr_arraysize, k);
    this->inMpr_var[k] = inMpr;
}

class MprFoundDescriptor : public cClassDescriptor
{
  public:
    MprFoundDescriptor();
    virtual ~MprFoundDescriptor();
...
Register_ClassDescriptor(MprFoundDescriptor);

MprFoundDescriptor::MprFoundDescriptor() : cClassDescriptor("inet::mpr_t2::MprFound", "cPacket")
{
}

MprFoundDescriptor::~MprFoundDescriptor()
{
}

bool MprFoundDescriptor::doesSupport(cObject *obj) const
{
    return dynamic_cast<MprFound *>(obj)!=NULL;
}

const char *MprFoundDescriptor::getProperty(const char *propertyname) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    return basedesc ? basedesc->getProperty(propertyname) : NULL;
}

int MprFoundDescriptor::getFieldCount(void *object) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    return basedesc ? 2+basedesc->getFieldCount(object) : 2;
}

unsigned int MprFoundDescriptor::getFieldTypeFlags(void *object, int field) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
...
const char *MprFoundDescriptor::getFieldName(void *object, int field) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
...
int MprFoundDescriptor::findField(void *object, const char *fieldName) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    int base = basedesc ? basedesc->getFieldCount(object) : 0;
    if (fieldName[0]=='s' && strcmp(fieldName, "sender")==0) return base+0;
    if (fieldName[0]=='i' && strcmp(fieldName, "inMpr")==0) return base+1;
    return basedesc ? basedesc->findField(object, fieldName) : -1;
}

const char *MprFoundDescriptor::getFieldTypeString(void *object, int field) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
...
const char *MprFoundDescriptor::getFieldProperty(void *object, int field, const char *propertyname) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
...
int MprFoundDescriptor::getArraySize(void *object, int field) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
    ...
    MprFound *pp = (MprFound *)object; (void)pp;
    switch (field) {
        case 1: return pp->getInMprArraySize();
        default: return 0;
    }
}

std::string MprFoundDescriptor::getFieldAsString(void *object, int field, int i) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
    ...
    switch (field) {
        case 0: return oppstring2string(pp->getSender());
        case 1: return oppstring2string(pp->getInMpr(i));
        default: return "";
    }
}

bool MprFoundDescriptor::setFieldAsString(void *object, int field, int i, const char *value) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
    ...
    switch (field) {
        case 0: pp->setSender((value)); return true;
        case 1: pp->setInMpr(i,(value)); return true;
        default: return false;
    }
}

const char *MprFoundDescriptor::getFieldStructName(void *object, int field) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
...
void *MprFoundDescriptor::getFieldStructPointer(void *object, int field, int i) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
