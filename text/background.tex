...
\begin{figure*}[t]
	\centering
	\subfigure[Backbone of 3 nodes shown by the black filled-in circles.]{\label{fig:explaining-backbone:a}\includegraphics[width=0.4\textwidth]{fig/explainingbackbone_b.pdf}}
	~
...
%
Firstly, any one-hop neighbor $n \in N$ is chosen as relay nodes of the MPR set if $n$ is an unique neighbor of some other node at $N^{2}$.
%
Secondly, while there exists uncovered nodes in $N^{2}$ by the current MPR set, we iterate over each remaining neighbor $n \in N$ and we count how many two-hop neighbors are cover by $n$.
%
Finally, $n$ is a relay node only if the number of covered two-hop neighbors is maximal.

%
Figure~\ref{fig:explaining-backbone:b} shows how node $A$ discover its MPR set. In the first step, 
